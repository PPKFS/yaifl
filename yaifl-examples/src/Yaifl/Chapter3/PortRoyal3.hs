module Yaifl.Chapter3.PortRoyal3 where
...
Lime Street, wider and healthier but not as rich, runs directly south, and to the north the road opens up into the courtyard of Fort James.|]
  ...
  ls `isSouthOf` ts
  fr <- addRoom "Fisher's Row" $ newRoom
...
Port Royal from its earliest days, it is nearly the largest building in the town.|]
  ...
  tsawb <- addRoom "Thames Street at Wherry Bridge" $ newRoom & #description .~
  ...
  tpa <- addRoom "The Private Alley" $ newRoom & #description .~ [wrappedText|You're just outside the tavern the Feathers. To the north, under a
...
little distance to the south.|]
  ...
  tf <- addRoom "The Feathers" $ newRoom & #description .~ [wrappedText|Newly built with brick, replacing the older Feathers tavern that used to stand here.
...
There's a room upstairs for those wanting to stay the night.|]
  ...
  tsbtkh <- addRoom "Thames Street by the King's House" $ newRoom & #description .~ [wrappedText|The King's House is reserved for the use of the Governor, but he does not live in it,
...
establishment on Thames Street, and the crowd -- which, to the west, is extremely dense -- here thins out a bit.|]
  ...
  tsbfc <- addRoom "Thames Street before Fort Carlisle" $ newRoom & #description .~ [wrappedText|Here Thames Street, formerly a respectable width, narrows to a footpath in order to
...
water beyond, and the Blue Mountains rising on the other side of the harbour.|]
  ...
  fc <- addRoom "Fort Carlisle" $ newRoom & #description .~ [wrappedText|Handsomely arrayed with cannons which you could fire at any moment -- though of course
