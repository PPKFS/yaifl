...
runRulebookEx :: WithGameData w m => Rulebook w s v -> m (Maybe s, Maybe v)
runRulebookEx (RulebookWithVariables n def i r) =
    if null r
        then pure (Nothing, def)
        else do
            -- logRulebookName n
            --attempt to set the rulebook variables
            iv <- liftWorld i
            res <-
                maybe --if we were not able to set them
                    ( do
                        logError "Could not successfully parse arguments"
                        return def
                    )
                    (processRuleList r def)
                    iv
            -- logDebug $ "Finished following the " <> n <> " with result " <> maybe "nothing" show res
            let r' = res <|> def
            return (iv, r')
runRulebookEx (Rulebook n def r) =
    if null r
        then pure (Nothing, def)
        else do
            -- logRulebookName n
...
runRulebook :: WithGameData w m => Rulebook w s v -> m (Maybe v)
...
processRuleList (x : xs) def args = case x of
    ...
    RuleWithVariables _ (RuleVarsT rf) -> do
        ...
        (res, s) <- liftWorld (runStateT rf args)
